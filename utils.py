...
def plot_trajectory(poses):
    try:
        ...
        ax.set_ylabel('Y')
        ax.set_zlabel('Z')
        ax.set_xticks(np.linspace(np.min(xyz[:, 0]), np.max(xyz[:, 0]), 10))
        ax.set_yticks(np.linspace(np.min(xyz[:, 1]), np.max(xyz[:, 1]), 10))
        ax.set_zticks(np.linspace(np.min(xyz[:, 2]), np.max(xyz[:, 2]), 10))
